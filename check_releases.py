...
# Стоп-слова: если они есть в новости — пропускаем (это не релиз)
STOPWORDS = [
    "вебинар",
    "курс",
    "приглашает",
...
def matches(text):
    # Сначала проверяем стоп-слова: если нашли — сразу пропускаем
    for stop in STOPWORDS:
        if stop.lower() in text.lower():
            return False

    # Теперь ищем слова наших конфигураций
    for kw in KEYWORDS:
